...
                        type: "data",
                        value:
                          "Günlük çıktısı depolama sınırını (10KB) aştı. Tam çıktı kullanıcıya gösterildi ancak sunucu depolaması için kısaltıldı.",
                      },
                    ],
                  },
                ]
...
        }),
        guide:
          "Yürütme tamamlandı. Sağlayın: 1) Ana sonuçlar/çıktılar 2) Önemli içgörüler veya bulgular 3) Varsa hata açıklamaları. Kodu veya ham günlükleri tekrarlamayın - kullanıcı için yorumlayın ve özetleyin.",
      });
    },
    [onResult],
  );
...
                return <img key={i} src={arg.value} alt="Kod çıktısı" />;
              }
              return (
                <span key={i}>
                  {isString(arg?.value)
...
      {
        type: "log",
        args: [{ type: "data", value: "Kod yeniden yürütülüyor..." }],
        time: Date.now(),
      },
    ]);
...
          <TextShimmer className="text-xs">Kod Oluşturuluyor...</TextShimmer>
        </>
      );
    return (
      <>
...
            <span className="text-destructive text-xs">HATA</span>
          </>
        ) : (
